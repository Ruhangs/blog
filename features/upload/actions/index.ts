...
const uploadToQiniu = async (input: string): Promise<string> => {
  ...
  // 文件上传
  const res: { data: { key: string } } = await formUploader.putFile(
    uploadToken,
    fileName,
    inputFilePath,
    putExtra,
    function (respErr, respBody, respInfo: { statusCode: number }) {
      if (respErr) {
        throw respErr;
      }
  ...
  const url = OSS_UPLOAD_DIR + res.data.key;
  return url;
};
...
export const uploadFile = async (
  ...
  if (isProduction()) {
    ...
    // TODO 上传同一个图片应该覆盖，去掉hash？
    const qiniuURL: string = await uploadToQiniu(localFileUrl);
    // 删除本地的压缩过后的图片文件
